/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import * as components from "../components";
import * as z from "zod";

export type StatusRequest = {
    /**
     * The Team identifier to perform the request on behalf of.
     */
    teamId?: string | undefined;
    /**
     * The Team slug to perform the request on behalf of.
     */
    slug?: string | undefined;
};

export enum StatusStatus {
    Disabled = "disabled",
    Enabled = "enabled",
    OverLimit = "over_limit",
    Paused = "paused",
}

export type StatusResponseBody = {
    status: StatusStatus;
};

export type StatusResponse = {
    httpMeta: components.HTTPMetadata;
    object?: StatusResponseBody | undefined;
};

/** @internal */
export namespace StatusRequest$ {
    export const inboundSchema: z.ZodType<StatusRequest, z.ZodTypeDef, unknown> = z
        .object({
            teamId: z.string().optional(),
            slug: z.string().optional(),
        })
        .transform((v) => {
            return {
                ...(v.teamId === undefined ? null : { teamId: v.teamId }),
                ...(v.slug === undefined ? null : { slug: v.slug }),
            };
        });

    export type Outbound = {
        teamId?: string | undefined;
        slug?: string | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, StatusRequest> = z
        .object({
            teamId: z.string().optional(),
            slug: z.string().optional(),
        })
        .transform((v) => {
            return {
                ...(v.teamId === undefined ? null : { teamId: v.teamId }),
                ...(v.slug === undefined ? null : { slug: v.slug }),
            };
        });
}

/** @internal */
export const StatusStatus$: z.ZodNativeEnum<typeof StatusStatus> = z.nativeEnum(StatusStatus);

/** @internal */
export namespace StatusResponseBody$ {
    export const inboundSchema: z.ZodType<StatusResponseBody, z.ZodTypeDef, unknown> = z
        .object({
            status: StatusStatus$,
        })
        .transform((v) => {
            return {
                status: v.status,
            };
        });

    export type Outbound = {
        status: StatusStatus;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, StatusResponseBody> = z
        .object({
            status: StatusStatus$,
        })
        .transform((v) => {
            return {
                status: v.status,
            };
        });
}

/** @internal */
export namespace StatusResponse$ {
    export const inboundSchema: z.ZodType<StatusResponse, z.ZodTypeDef, unknown> = z
        .object({
            HttpMeta: components.HTTPMetadata$.inboundSchema,
            object: z.lazy(() => StatusResponseBody$.inboundSchema).optional(),
        })
        .transform((v) => {
            return {
                httpMeta: v.HttpMeta,
                ...(v.object === undefined ? null : { object: v.object }),
            };
        });

    export type Outbound = {
        HttpMeta: components.HTTPMetadata$.Outbound;
        object?: StatusResponseBody$.Outbound | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, StatusResponse> = z
        .object({
            httpMeta: components.HTTPMetadata$.outboundSchema,
            object: z.lazy(() => StatusResponseBody$.outboundSchema).optional(),
        })
        .transform((v) => {
            return {
                HttpMeta: v.httpMeta,
                ...(v.object === undefined ? null : { object: v.object }),
            };
        });
}
