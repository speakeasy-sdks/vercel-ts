/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { HTTPMetadata, HTTPMetadata$ } from "./httpmetadata";
import * as z from "zod";

/**
 * An object describing the reason why the team is being deleted.
 */
export type Reasons = {
    /**
     * Idenitifier slug of the reason why the team is being deleted.
     */
    slug: string;
    /**
     * Description of the reason why the team is being deleted.
     */
    description: string;
};

export type DeleteTeamRequestBody = {
    /**
     * Optional array of objects that describe the reason why the team is being deleted.
     */
    reasons?: Array<Reasons> | undefined;
};

export type DeleteTeamRequest = {
    /**
     * The Team identifier to perform the request on behalf of.
     */
    teamId: string;
    /**
     * Id of the team to be set as the new default team
     */
    newDefaultTeamId?: string | undefined;
    /**
     * The Team slug to perform the request on behalf of.
     */
    slug?: string | undefined;
    requestBody?: DeleteTeamRequestBody | undefined;
};

/**
 * The Team was successfully deleted
 */
export type DeleteTeamResponseBody = {
    /**
     * The ID of the deleted Team
     */
    id: string;
    /**
     * Signifies whether the default team update has failed, when newDefaultTeamId is provided in request query.
     */
    newDefaultTeamIdError?: boolean | undefined;
};

export type DeleteTeamResponse = {
    httpMeta: HTTPMetadata;
    /**
     * The Team was successfully deleted
     */
    object?: DeleteTeamResponseBody | undefined;
};

/** @internal */
export namespace Reasons$ {
    export const inboundSchema: z.ZodType<Reasons, z.ZodTypeDef, unknown> = z
        .object({
            slug: z.string(),
            description: z.string(),
        })
        .transform((v) => {
            return {
                slug: v.slug,
                description: v.description,
            };
        });

    export type Outbound = {
        slug: string;
        description: string;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, Reasons> = z
        .object({
            slug: z.string(),
            description: z.string(),
        })
        .transform((v) => {
            return {
                slug: v.slug,
                description: v.description,
            };
        });
}

/** @internal */
export namespace DeleteTeamRequestBody$ {
    export const inboundSchema: z.ZodType<DeleteTeamRequestBody, z.ZodTypeDef, unknown> = z
        .object({
            reasons: z.array(z.lazy(() => Reasons$.inboundSchema)).optional(),
        })
        .transform((v) => {
            return {
                ...(v.reasons === undefined ? null : { reasons: v.reasons }),
            };
        });

    export type Outbound = {
        reasons?: Array<Reasons$.Outbound> | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, DeleteTeamRequestBody> = z
        .object({
            reasons: z.array(z.lazy(() => Reasons$.outboundSchema)).optional(),
        })
        .transform((v) => {
            return {
                ...(v.reasons === undefined ? null : { reasons: v.reasons }),
            };
        });
}

/** @internal */
export namespace DeleteTeamRequest$ {
    export const inboundSchema: z.ZodType<DeleteTeamRequest, z.ZodTypeDef, unknown> = z
        .object({
            teamId: z.string(),
            newDefaultTeamId: z.string().optional(),
            slug: z.string().optional(),
            RequestBody: z.lazy(() => DeleteTeamRequestBody$.inboundSchema).optional(),
        })
        .transform((v) => {
            return {
                teamId: v.teamId,
                ...(v.newDefaultTeamId === undefined
                    ? null
                    : { newDefaultTeamId: v.newDefaultTeamId }),
                ...(v.slug === undefined ? null : { slug: v.slug }),
                ...(v.RequestBody === undefined ? null : { requestBody: v.RequestBody }),
            };
        });

    export type Outbound = {
        teamId: string;
        newDefaultTeamId?: string | undefined;
        slug?: string | undefined;
        RequestBody?: DeleteTeamRequestBody$.Outbound | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, DeleteTeamRequest> = z
        .object({
            teamId: z.string(),
            newDefaultTeamId: z.string().optional(),
            slug: z.string().optional(),
            requestBody: z.lazy(() => DeleteTeamRequestBody$.outboundSchema).optional(),
        })
        .transform((v) => {
            return {
                teamId: v.teamId,
                ...(v.newDefaultTeamId === undefined
                    ? null
                    : { newDefaultTeamId: v.newDefaultTeamId }),
                ...(v.slug === undefined ? null : { slug: v.slug }),
                ...(v.requestBody === undefined ? null : { RequestBody: v.requestBody }),
            };
        });
}

/** @internal */
export namespace DeleteTeamResponseBody$ {
    export const inboundSchema: z.ZodType<DeleteTeamResponseBody, z.ZodTypeDef, unknown> = z
        .object({
            id: z.string(),
            newDefaultTeamIdError: z.boolean().optional(),
        })
        .transform((v) => {
            return {
                id: v.id,
                ...(v.newDefaultTeamIdError === undefined
                    ? null
                    : { newDefaultTeamIdError: v.newDefaultTeamIdError }),
            };
        });

    export type Outbound = {
        id: string;
        newDefaultTeamIdError?: boolean | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, DeleteTeamResponseBody> = z
        .object({
            id: z.string(),
            newDefaultTeamIdError: z.boolean().optional(),
        })
        .transform((v) => {
            return {
                id: v.id,
                ...(v.newDefaultTeamIdError === undefined
                    ? null
                    : { newDefaultTeamIdError: v.newDefaultTeamIdError }),
            };
        });
}

/** @internal */
export namespace DeleteTeamResponse$ {
    export const inboundSchema: z.ZodType<DeleteTeamResponse, z.ZodTypeDef, unknown> = z
        .object({
            HttpMeta: HTTPMetadata$.inboundSchema,
            object: z.lazy(() => DeleteTeamResponseBody$.inboundSchema).optional(),
        })
        .transform((v) => {
            return {
                httpMeta: v.HttpMeta,
                ...(v.object === undefined ? null : { object: v.object }),
            };
        });

    export type Outbound = {
        HttpMeta: HTTPMetadata$.Outbound;
        object?: DeleteTeamResponseBody$.Outbound | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, DeleteTeamResponse> = z
        .object({
            httpMeta: HTTPMetadata$.outboundSchema,
            object: z.lazy(() => DeleteTeamResponseBody$.outboundSchema).optional(),
        })
        .transform((v) => {
            return {
                HttpMeta: v.httpMeta,
                ...(v.object === undefined ? null : { object: v.object }),
            };
        });
}
