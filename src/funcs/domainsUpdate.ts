...
import {
  PatchDomainRequest,
  PatchDomainRequest$outboundSchema,
  PatchDomainRequestBody,
  PatchDomainResponseBody,
  PatchDomainResponseBody$inboundSchema,
...
export async function domainsUpdate(
  ...
  >
> {
  const input$: PatchDomainRequest = {
    domain: domain,
    teamId: teamId,
    slug: slug,
  ...
  const parsed$ = schemas$.safeParse(
    input$,
    (value$) => PatchDomainRequest$outboundSchema.parse(value$),
  ...
  const path$ = pathToFunc("/v3/domains/{domain}")(pathParams$);
  ...
  const [result$] = await m$.match<
    ...
    | ConnectionError
  >(
    m$.json(200, PatchDomainResponseBody$inboundSchema),
