...
async function run() {
  const result = await sdk.integrations.get("icfg_cuwj0AdCdH3BwWT4LPijCC7t", "<value>", "<value>");
...
async function run() {
  const result = await sdk.integrations.deleteConfiguration("<value>", "<value>", "<value>");
...
```typescript
import { SDK } from "openapi";
import { Provider } from "openapi/models/operations";
...
async function run() {
  const result = await sdk.integrations.listGitNamespaces("ghes-test.now.systems", Provider.Gitlab, "<value>", "<value>");
...
async function run() {
  const result = await sdk.integrations.listGitRepos({
