...
async function run() {
  const result = await sdk.artifacts.status("<value>", "<value>");
...
async function run() {
  const result = await sdk.artifacts.download({
    xArtifactClientCi: "VERCEL",
    xArtifactClientInteractive: 0,
    hash: "12HKQaOmR5t5Uy6vdcQsNIiZgHGB",
  });
...
async function run() {
  await sdk.artifacts.exists("12HKQaOmR5t5Uy6vdcQsNIiZgHGB", "<value>", "<value>");
...
async function run() {
  const result = await sdk.artifacts.query("<value>", "<value>", {
    hashes: [
      "<value>",
    ],
  });
